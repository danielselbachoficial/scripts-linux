# 🐧 scripts-linux
Uma coleção de ferramentas de automação, hardening e deploy para profissionais de infraestrutura. Este repositório é projetado para Sysadmins, Administradores de Redes, Analistas de NOC, Especialistas em Cibersegurança e Pentesters.

## 🎯 Público Alvo & Casos de Uso
- **Sysadmins:** Automação de deploys complexos e gestão de serviços.
- **Analistas de Redes/NOC:** Instalação de ferramentas IPAM/DCIM (NetBox) e monitoramento.
- **Cybersecurity/Blue Team:** Scripts de hardening de sistema, firewall e auditoria.
- **Pentesters:** Referência de configurações seguras e análise de superfícies de ataque.

## 📂 Organização por Especialidade

### 🌐 Infra & NOC (/services-noc)
Focado em serviços essenciais para gestão de infraestrutura de rede.

- **NetBox:** Deploy automatizado com foco em segurança máxima (IPAM/DCIM).

### 🛡️ Cybersecurity & Hardening (/security)
Scripts para elevar o nível de segurança do sistema operacional.

- **OS Hardening:** Configurações de kernel, limites de processo e permissões.
- **Network Defense:** Configuração de UFW, Fail2ban e políticas de TLS.

### ⚙️ Administração de Sistemas (/sysadmin)
Utilitários de manutenção e automação de rotinas.

- **Backups:** Scripts para dumps de BD e sincronização off-site.
- **Performance:** Monitoramento de recursos e limpeza de logs.
